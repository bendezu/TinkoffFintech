...
fun SharedPreferences.getUser(): User {
    val id = getLong(PREF_ID, 0)
    val email = getString(PREF_EMAIL, "").orEmpty()
    val first = getString(PREF_FIRST_NAME, "").orEmpty()
    val second = getString(PREF_SECOND_NAME, "").orEmpty()
    val middle = getString(PREF_PATRONYMIC, "").orEmpty()
    val avatar = getString(PREF_AVATAR, null)
    return User(id, email, first, second, middle, avatar)
}
